 ...
 *8.互斥锁：只用于单个资源之间
 *10.增加了kmalloc动态内存分配的使用,基本就是将变量gmydev改为指针pgmydev
 * */
...
//10.1
struct mychar_dev *pgmydev=NULL;//全局变量就只使用在入口函数这里，如init等等！
...
{ 
	...
	if(NULL==pgmydev){
		//10.3 虽然申请内存失败，但是前面申请设备号成功要将设备号归还给系统！
		unregister_chrdev_region(devno,mychar_num);//将分配好的设备号归还给系统
	...
	cdev_add(&pgmydev->mydev,devno,mychar_num);//将我们的对象加入到我们内核管理设备的哈希链表里边去
	/* 这里进行阻塞非阻塞初始化 */
	init_waitqueue_head(&pgmydev->rq);
	init_waitqueue_head(&pgmydev->wq);/* 接下来改写读写函数 */
