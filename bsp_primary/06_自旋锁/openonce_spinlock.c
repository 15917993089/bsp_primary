...
int openonce_open(struct inode *pnode,struct file *pfile){//要使与设备相关的几个函数不使用全局变量！例如：read阿write啊！
	...
	//6.3
	if(pmydev->openflag){
		pmydev->openflag=0;//其他进程不能打开
		spin_unlock(&pmydev->lock);//打开使用之后归还
		return 0;
	}else{
		spin_unlock(&pmydev->lock);//打开失败之后归还
		printk("The device is open already\n");
		return -1;
	}
#endif
...
int openonce_close(struct inode *pnode,struct file *pfile){
	...
	//6.4 这样对openflag的访问就不会出现同时
	spin_lock(&pmydev->lock);
	pmydev->openflag=1;//可打开的时候在关闭的时候把值置为1
